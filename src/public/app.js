...
function connectWebSocket() {
    ...
    // Escuchar alertas de rate limit
    socket.on('rate_limit_alert', (data) => {
        console.warn('🚨 ALERTA DE RATE LIMIT:', data);
...
// Mostrar notificación de alerta de rate limit
function showRateLimitNotification(data) {
    // Crear elemento de notificación si no existe
    let notification = document.getElementById('rate-limit-notification');
        ...
        <div style="display: flex; align-items: start; gap: 12px;">
            ...
            <div>
                ...
                <div style="font-size: 14px; opacity: 0.95; margin-bottom: 8px;">
                    Uso de API al ${data.usage}% (${data.calls}/${data.limit} llamadas/hora)
                </div>
    ...
    // Al hacer clic, abrir el modal de rate limits
...
if (!document.getElementById('rate-limit-animations')) {
...
// ==================== RATE LIMIT MODAL ====================
window.openRateLimitModal = async function() {
    dropdownMenu.classList.remove('show');
...
window.refreshRateLimitStats = async function() {
    await loadRateLimitStats();
};

async function loadRateLimitStats() {
    const currentInfo = document.getElementById('rate-limit-current');
    const hourlyInfo = document.getElementById('rate-limit-hourly');

    // Mostrar loading
    currentInfo.innerHTML = '<p class="loading">Cargando...</p>';
    hourlyInfo.innerHTML = '<p class="loading">Cargando...</p>';
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error('Error al cargar estadísticas de rate limit');
        }
        ...
        if (usagePercentage >= 70) {
            barClass = 'danger';
            statusIcon = '🚨';
            alertHtml = '<div class="alert-box"><p>⚠️ <strong>ALERTA:</strong> Has superado el 70% del límite. Monitorea de cerca el uso.</p></div>';
        } else if (usagePercentage >= 50) {
            barClass = 'warning';
            statusIcon = '⚠️';
            alertHtml = '<div class="alert-box warning"><p>⚠️ Uso moderado. Mantén un seguimiento del consumo.</p></div>';
        } else {
            statusIcon = '✅';
            alertHtml = '<div class="alert-box info"><p>✅ Uso normal. Todo bajo control.</p></div>';
        }

        currentInfo.innerHTML = `
            ${alertHtml}
            <div class="rate-limit-stats-grid">
                <div class="rate-limit-stat-item">
                    <div class="rate-limit-stat-label">Llamadas Última Hora</div>
                    <div class="rate-limit-stat-value">${stats.callsLastHour} / ${stats.limit}</div>
                </div>
                <div class="rate-limit-stat-item">
                    <div class="rate-limit-stat-label">Porcentaje de Uso</div>
                    <div class="rate-limit-stat-value">${statusIcon} ${usagePercentage}%</div>
                </div>
                <div class="rate-limit-stat-item">
                    <div class="rate-limit-stat-label">Últimos 5 Minutos</div>
                    <div class="rate-limit-stat-value">${stats.callsLast5Minutes} calls</div>
                </div>
                <div class="rate-limit-stat-item">
                    <div class="rate-limit-stat-label">Último Minuto</div>
                    <div class="rate-limit-stat-value">${stats.callsLastMinute} calls</div>
                </div>
                <div class="rate-limit-stat-item">
                    <div class="rate-limit-stat-label">Proyección Horaria</div>
                    <div class="rate-limit-stat-value">${stats.projectedHourlyCalls} calls</div>
                </div>
                <div class="rate-limit-stat-item">
                    <div class="rate-limit-stat-label">Llamadas Restantes</div>
                    <div class="rate-limit-stat-value">${stats.remainingCalls}</div>
                </div>
            </div>
            <div class="progress-bar-container" style="margin-top: 20px;">
                <div class="progress-bar">
                    <div class="progress-bar-fill ${barClass}" style="width: ${usagePercentage}%"></div>
                </div>
                <div class="progress-bar-label">${usagePercentage}% del límite usado</div>
            </div>
        `;
        ...
        hourlyInfo.innerHTML = `
            <div class="stats-grid">
                <div class="stat-card">
                    ...
                    <div class="stat-card-value" style="font-size: 18px;">${hourlyStats.currentHour}</div>
                </div>
                <div class="stat-card">
                    <div class="stat-card-label">Total Llamadas</div>
                    <div class="stat-card-value">${hourlyStats.totalCalls}</div>
                </div>
                <div class="stat-card">
                    <div class="stat-card-label">Envíos de Mensajes</div>
                    <div class="stat-card-value">${hourlyStats.sendMessageCalls}</div>
                </div>
                <div class="stat-card">
                    <div class="stat-card-label">Descargas de Media</div>
                    <div class="stat-card-value">${hourlyStats.mediaDownloadCalls}</div>
                </div>
                <div class="stat-card">
                    <div class="stat-card-label">Pico por Minuto</div>
                    <div class="stat-card-value">${hourlyStats.peakCallsPerMinute}</div>
                </div>
                <div class="stat-card">
                    <div class="stat-card-label">Otras Llamadas</div>
                    <div class="stat-card-value">${hourlyStats.otherCalls}</div>
                </div>
            </div>
        `;

    } catch (error) {
        console.error('Error al cargar estadísticas de rate limit:', error);
        currentInfo.innerHTML = '<p class="modal-description" style="color: #ef4444;">Error al cargar estadísticas de uso actual</p>';
        hourlyInfo.innerHTML = '<p class="modal-description" style="color: #ef4444;">Error al cargar estadísticas de la hora</p>';
    }
}
